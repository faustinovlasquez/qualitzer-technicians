...
import { ActivityIndicator, BackHandler, Modal, Platform, Pressable, StyleSheet, Text, View, useWindowDimensions } from "react-native";
...
function Application({ app }: { app: ReturnType<typeof useTechnicianApp> }) {
...
function ApplicationRoot() {
  const app = useTechnicianApp();
  const { width } = useWindowDimensions();
  const reserveQrSpace = __DEV__ && Platform.OS === "web" && width >= 1100;
  ...
  return <View style={[styles.app, reserveQrSpace && { paddingRight: 184 }]}>
    <View style={styles.body}><Application app={app} /></View>
    ...
    {__DEV__ && <DevelopmentQrPanel gatewayUrl={app.gatewayUrl} />}
  </View>;
}
export default function App() { return <SafeAreaProvider><StatusBar style="dark" /><AppErrorBoundary><ApplicationRoot /></AppErrorBoundary></SafeAreaProvider>; }
