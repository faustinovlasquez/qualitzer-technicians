...
export function createNotificationAdapter(): NotificationAdapter {
  ...
  async function permission(request: boolean): Promise<NotificationPermission> {
    ...
    const result = request
      ? await module.requestPermissionsAsync({ ios: { allowAlert: true, allowSound: true, allowBadge: false } })
  ...
  return {
    ...
    async prepareChannel() {
      ...
      if (nativePlatform === "android") await module.setNotificationChannelAsync("technical-work", {
        name: "Trabajos técnicos", importance: module.AndroidImportance.DEFAULT, sound: "default",
        description: "Asignaciones y recordatorios de cronómetros activos", lockscreenVisibility: module.AndroidNotificationVisibility.PRIVATE,
      });
    },
    ...
    subscribe(listeners) {
      ...
      return () => {
        disposed = true;
        if (foregroundOwner === owner) { foregroundOwner = null; clearHandler?.(); }
