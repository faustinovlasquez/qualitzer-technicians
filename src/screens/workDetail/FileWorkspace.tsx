...
function FileWorkspaceContent(props: FileWorkspaceProps) {
  ...
  async function pick(source: FileSource): Promise<void> {
    ...
    try {
      ...
      if (callbacks.current.readOnly || callbacks.current.busy || callbacks.current.offline === null || callbacks.current.offline?.authBlocked) throw new Error("La selección se canceló porque este destino ya no permite cambios en este momento.");
  ...
  const connection = props.offline === null ? "Recuperando cola…" : props.offline?.authBlocked ? "Verifica tu sesión · pendientes conservados" : props.mode === "demo" ? "Demostración · guardado local" : props.offline === undefined ? "Carga manual · espera confirmación" : !props.offline.online ? "Sin conexión verificada · guardar en cola" : "Guardado explícito · sincronización automática";
  ...
  return <View style={props.compact ? workspaceStyles.bounded : styles.stack} testID="file-workspace">
    <View style={workspaceStyles.toolbar} testID="files-toolbar">
      ...
      {!props.readOnly ? <View style={workspaceStyles.controls}>
        <Button title="Cámara" icon="camera-outline" variant="secondary" style={workspaceStyles.compactPicker} disabled={unavailable || draft.files.length >= MAX_FILES} onPress={() => { void pick("camera"); }} />
        <Button title="Galería" icon="images-outline" variant="secondary" style={workspaceStyles.compactPicker} disabled={unavailable || draft.files.length >= MAX_FILES} onPress={() => { void pick("library"); }} />
        <Button title="Archivos" icon="folder-open-outline" variant="secondary" style={workspaceStyles.compactPicker} disabled={unavailable || draft.files.length >= MAX_FILES} onPress={() => { void pick("document"); }} />
      </View> : null}
    </View>
