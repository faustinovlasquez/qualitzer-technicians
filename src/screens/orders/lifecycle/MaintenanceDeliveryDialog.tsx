...
import type { SignaturePadHandle } from "./signatureGeometry";
...
export function MaintenanceDeliveryDialog(props: MaintenanceDeliveryDialogProps) {
  ...
  async function prepare(): Promise<void> {
    ...
    setCaptureError(null);
    try {
      if (!technicianPad.current || (clientRequired && !clientPad.current)) throw new Error("El área de firma no está disponible. Vuelve a abrir la entrega.");
      const technicianSignature = await technicianPad.current.capture();
        ...
        <ScrollView ref={scroll} keyboardShouldPersistTaps="handled" scrollEnabled={!drawing} contentContainerStyle={styles.content}>
          ...
          {review ? <View style={styles.stack}>
            ...
            <View style={styles.tight}>
              <Text style={styles.label}>Firma del técnico · {props.technicianName}</Text>
            ...
            <SignatureField label="Firma del técnico" name={props.technicianName} strokes={draft.technicianStrokes} signatureRef={technicianPad} disabled={blocked} error={errors.technicianSignature} onChange={(technicianStrokes) => update({ technicianStrokes })} onDrawingChange={setDrawing} />
            ...
            <BodyText>Las firmas se generan como PNG reales (hasta 768 × 320, máximo 1 MiB cada una). No se guardan en archivos ni almacenamiento persistente del dispositivo.</BodyText>
          </View>}
