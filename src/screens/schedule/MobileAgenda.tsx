...
export function MobileAgenda({ days, selectedDay, mode, clock, unavailableDates, unscheduled, busy, onMode, onSelect, onOpen }: MobileAgendaProps) {
  ...
  return <View style={styles.root} testID="mobile-agenda" accessibilityState={{ busy }}>
    <View style={styles.days} testID="agenda-day-strip">
      {days.map((day) => {
        ...
        return <Pressable key={day.date} accessibilityRole="button" accessibilityLabel={`${scheduleDayLabel(day.date)}. ${agendaDaySummary(day, missing)}${clock?.day === day.date ? ". Hoy" : ""}${selected ? ". Seleccionado" : ""}`} accessibilityState={{ selected, disabled: busy }} disabled={busy} onPress={() => { onSelect(day.date); onMode("day"); }} style={[styles.day, selected && styles.active, busy && styles.disabled]}>
          ...
          <View style={[styles.dot, { backgroundColor: missing || day.overlapMinutes > 0 ? palette.amber : day.blocks.length || clock?.day === day.date ? selected ? palette.white : palette.primary : "transparent" }]} />
        </Pressable>;
      })}
    </View>
...
const styles = StyleSheet.create({
  ...
  days: { flexDirection: "row", gap: 2, width: "100%" },
  day: { flex: 1, minWidth: 0, minHeight: 64, borderRadius: 10, alignItems: "center", justifyContent: "center", gap: 3, paddingVertical: 6 },
  weekday: { fontSize: 12, color: palette.textSecondary }, dayNumber: { fontSize: 18, fontWeight: "700", color: palette.text },
