...
import { assignmentDay, assignmentIncludesDay, assignmentPlannedMinutes, assignmentWorkForDay, dailyRange } from "../domain/assignmentSchedule";
...
function Kpi({ title, value, note, icon, tone }: { title: string; value: number | null; note: string; icon: IconName; tone: "warning" | "teal" | "success" }) {
  ...
  return (
    <Card style={styles.kpi}>
      <View style={[styles.kpiIcon, { backgroundColor: background }]}><Ionicons name={icon} size={18} color={color} accessible={false} /></View>
...
export function DashboardScreen({ data, user, range, loading, error, onRefresh, onRangeChange, onOpenWork, onOpenGroup, onWorkStatus, busy = false, serverRemindersReady = false, offline, companyBranchId, focusDate, onFocusDate, view }: DashboardScreenProps) {
  ...
  const heroTitle = coveragePending ? "Verificando datos locales" : partial ? "Jornada con cobertura parcial" : !hasData
  ...
  const heroDescription = coveragePending ? "La cobertura y los pendientes aún no están verificados."
    : partial ? "Solo se muestran datos disponibles en este dispositivo. Los días sin copia no equivalen a días sin tareas."
    : !hasData
    ...
    : counts.total === 0 ? "No hay tareas asignadas para el período seleccionado."
      : `${counts.completed} de ${counts.total} ${counts.total === 1 ? "tarea completada" : "tareas completadas"}. ${selectedDay ? "Tu día" : "Tu semana"}, bajo control.`;
  ...
  if (view === "agenda" && agendaLayout === "schedule") {
    ...
    return compact ? <ScrollView style={styles.screen} contentContainerStyle={styles.mobileSchedule} keyboardShouldPersistTaps="handled" refreshControl={<RefreshControl refreshing={loading} onRefresh={onRefresh} tintColor={palette.primary} />}>{content}</ScrollView> : <View style={styles.desktopSchedule}>{content}</View>;
  }
  ...
  return (
    ...
    >
      ...
      <View style={styles.heading}>
        ...
        <Text style={styles.greeting}>{firstName ? `Hola, ${firstName}.` : "Hola."} {fullDate(today)}</Text>
      </View>

      <LinearGradient colors={[palette.navy, "#174C57"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.hero}>
        <View pointerEvents="none" style={styles.heroOrbit} />
        <View style={styles.heroTop}>
          <Text style={styles.heroEyebrow}>{selectedDay ? "JORNADA SELECCIONADA" : "TU SEMANA EN CAMPO"}</Text>
          <View style={styles.heroIcon}><Ionicons name="construct-outline" size={22} color="#ADE5D9" accessible={false} /></View>
        </View>
        <Text style={styles.heroTitle}>{heroTitle}</Text>
        <Text style={styles.heroDescription}>{heroDescription}</Text>
        <View style={styles.heroBottom}>
          <View style={styles.heroStat}>
            <Ionicons name="calendar-outline" size={16} color={palette.onDark} accessible={false} />
            <Text style={styles.heroStatText}>{selectedDay ? fullDate(selectedDay) : rangeLabel}</Text>
          </View>
          {hasData && !coveragePending ? <View style={styles.heroStat}><Ionicons name="time-outline" size={16} color={palette.onDark} accessible={false} /><Text style={styles.heroStatText}>{partial && counts.total === 0 ? "Carga no disponible" : `${duration(counts.minutes)} planificadas${partial ? " · parcial" : ""}`}</Text></View> : null}
        </View>
      </LinearGradient>

      <View style={styles.kpiRow}>
        <Kpi title="Pendientes" value={hasData && !coveragePending && (!partial || counts.pending > 0) ? counts.pending : null} note={partial ? "Conteo parcial" : "Por comenzar"} icon="hourglass-outline" tone="warning" />
        <Kpi title="En curso" value={hasData && !coveragePending && (!partial || counts.active > 0) ? counts.active : null} note={partial ? "Conteo parcial" : counts.paused > 0 ? `${counts.paused} en pausa` : "En ejecución"} icon="pulse-outline" tone="teal" />
        <Kpi title="Completadas" value={hasData && !coveragePending && (!partial || counts.completed > 0) ? counts.completed : null} note={partial ? "Conteo parcial" : counts.delivered > 0 ? `${counts.delivered} entregadas` : "Finalizadas"} icon="checkmark-done-outline" tone="success" />
      </View>
      ...
      <Card style={styles.weekCard}>
        <View style={styles.weekNavigation}>
          <IconButton name="chevron-back-outline" label="Semana anterior" onPress={() => navigateWeek(-1)} />
          <View style={styles.weekCaption}><Text style={styles.weekTitle}>Semana</Text><Text style={styles.weekRange}>{weekLabel}</Text></View>
          <IconButton name="chevron-forward-outline" label="Semana siguiente" onPress={() => navigateWeek(1)} />
        </View>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.weekDays}>
          {days.map((day) => {
            ...
            const disabled = unavailable && !local;
            ...
            return (
              <Pressable
                ...
                disabled={disabled}
                style={({ pressed }) => [styles.day, isToday && styles.dayToday, selected && styles.daySelected, pressed && styles.pressed]}
              >
                ...
                <View style={[styles.dayDot, isToday && styles.dayDotToday, selected && styles.dayDotSelected]} />
              </Pressable>
            );
          })}
        </ScrollView>
        <View style={styles.weekActions}>
          {view === "agenda" ? <Pressable accessibilityRole="button" accessibilityLabel="Mostrar todas las tareas de la semana" accessibilityState={{ selected: selectedDay === null }} onPress={() => setDaySelection(null)} style={({ pressed }) => [styles.textButton, pressed && styles.pressed]}>
            <Text style={styles.textButtonLabel}>Toda la semana</Text>
          </Pressable> : null}
          <Pressable accessibilityRole="button" accessibilityLabel={view === "today" ? "Ir a hoy" : "Ir a la semana actual"} onPress={() => { setDaySelection(null); setWeekSelection(null); onRangeChange(view === "today" ? dailyRange(today) : weekRange(today)); }} style={({ pressed }) => [styles.textButton, pressed && styles.pressed]}>
            <Ionicons name="today-outline" size={15} color={palette.primary} accessible={false} /><Text style={styles.textButtonLabel}>{view === "today" ? "Hoy" : "Semana actual"}</Text>
          </Pressable>
        </View>
      </Card>

      <View style={styles.taskHeading}>
        <SectionTitle title={view === "agenda" ? "Agenda de trabajo" : "Mis asignaciones"} subtitle={selectedDay ? `${fullDate(selectedDay)} y pendientes anteriores` : "Tareas de toda la semana y pendientes anteriores"} />
      ...
      {hasData && filteredEntries.length > 0 ? listView === "orders" ? <View style={styles.section}>
        ...
        <View key={section.key} style={styles.section}>
          ...
          {section.entries.map(({ group, work }) => <AssignmentWorkCard key={JSON.stringify([group.type, group.id, work.workType, work.id, work.scheduledDate])} group={group} work={work} onOpenWork={onOpenWork} onWorkStatus={onWorkStatus} busy={busy} generatedAt={data.generatedAt} online={offline?.online ?? !coveragePending} staleReadOnly={coveragePending || offline?.authBlocked === true || isPendingLocalWork(work)} />)}
        </View>
...
const styles = StyleSheet.create({
  ...
  content: { width: "100%", maxWidth: theme.contentWidth, alignSelf: "center", paddingHorizontal: 20, paddingTop: 12, paddingBottom: 96, gap: 20 },
  heading: { gap: 6 },
  pageTitle: { ...typography.title, color: palette.navy },
  greeting: { ...typography.body, fontSize: 14, color: palette.textSecondary },
  hero: { borderRadius: radius.lg, padding: 24, gap: 12, overflow: "hidden" },
  heroOrbit: { position: "absolute", right: -95, top: -95, width: 250, height: 250, borderRadius: 125, borderWidth: 1, borderColor: palette.darkBorder },
  heroTop: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", gap: 12 },
  heroEyebrow: { ...typography.overline, color: palette.onDark, flexShrink: 1 },
  heroIcon: { width: 40, height: 40, borderRadius: 13, backgroundColor: palette.darkSurface, alignItems: "center", justifyContent: "center" },
  heroTitle: { ...typography.title, fontSize: 27, color: palette.white, maxWidth: 520 },
  heroDescription: { ...typography.body, color: palette.onDark, maxWidth: 580 },
  heroBottom: { borderTopWidth: 1, borderTopColor: palette.darkBorder, paddingTop: 16, marginTop: 4, gap: 9, flexDirection: "row", flexWrap: "wrap", columnGap: 20 },
  heroStat: { flexDirection: "row", alignItems: "center", gap: 7, flexShrink: 1 },
  heroStatText: { ...typography.caption, color: palette.onDark, flexShrink: 1 },
  kpiRow: { flexDirection: "row", flexWrap: "wrap", gap: 10 },
  kpi: { flex: 1, minWidth: 96, padding: 13, gap: 5 },
  kpiIcon: { width: 32, height: 32, borderRadius: 10, alignItems: "center", justifyContent: "center", marginBottom: 4 },
  kpiValue: { fontSize: 28, lineHeight: 34, fontWeight: "800", color: palette.navy, fontVariant: ["tabular-nums"], letterSpacing: -0.8 },
  ...
  weekCard: { padding: 10, gap: 8 },
  weekNavigation: { flexDirection: "row", alignItems: "center", gap: 4 },
  weekCaption: { flex: 1, alignItems: "center", gap: 2 },
  weekTitle: { ...typography.label, color: palette.navy, fontWeight: "700" },
  weekRange: { ...typography.caption, color: palette.textSecondary, textAlign: "center" },
  weekDays: { flexGrow: 1, justifyContent: "space-between", gap: 4, paddingVertical: 4 },
  day: { minWidth: 44, minHeight: 80, flex: 1, alignItems: "center", justifyContent: "center", paddingHorizontal: 4, paddingVertical: 10, borderRadius: radius.md, borderWidth: 1, borderColor: "transparent", gap: 5 },
  ...
  dayCoverage: { fontSize: 10, color: palette.amber },
  ...
  dayDotToday: { backgroundColor: palette.primary },
  dayDotSelected: { backgroundColor: palette.white },
  weekActions: { flexDirection: "row", flexWrap: "wrap", alignItems: "center", justifyContent: "space-between", borderTopWidth: 1, borderTopColor: palette.track, paddingTop: 4 },
