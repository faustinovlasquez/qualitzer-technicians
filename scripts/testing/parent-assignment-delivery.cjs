...
async function main() {
  ...
  assert.equal(release.version, "1.0.21");
  assert.equal(release.versionCode, 22);
  const apk = fs.readFileSync(path.join(root, "artifacts/qualitzer-tecnicos-1.0.21-android.apk"));
  assert.equal(digest(apk), release.sha256);
  const artifact = "infrastructure/mobile-gateway/qualitzer-mobile-gateway-1.0.8.tgz";
  const archive = fs.readFileSync(path.join(backend, artifact));
  assert.equal(digest(archive), "f02562722c884db9046b54009a77dcbb8e06bd168246360e23fb248f4b8bbe85");
  ...
  assert.equal(lock.packages["node_modules/@qualitzer/mobile-gateway"].version, "1.0.8");
  ...
  assert.ok(/^http:\/\/192\.168\.1\.101:8795\/$/.test(url), "EXPECTED_LOCAL_DELIVERY_URL");
  const response = await fetch(`${url}qualitzer-tecnicos-1.0.21-android.apk`, { signal: AbortSignal.timeout(20000) });
  ...
  fs.writeFileSync(path.join(root, "artifacts/parent-assignment-delivery-1.0.21.json"), JSON.stringify(report, null, 2));
