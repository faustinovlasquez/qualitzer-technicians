...
async function main() {
  ...
  assert.equal(release.version, "1.0.17");
  assert.equal(release.versionCode, 18);
  const apk = fs.readFileSync(path.join(root, "artifacts/qualitzer-tecnicos-1.0.17-android.apk"));
  assert.equal(digest(apk), release.sha256);
  const artifact = "infrastructure/mobile-gateway/qualitzer-mobile-gateway-1.0.6.tgz";
  const archive = fs.readFileSync(path.join(backend, artifact));
  assert.equal(digest(archive), "769448926b4a75f2efcb6ddea8c383d7959b862d711c0c8d229d0daa6e487bf3");
  ...
  assert.equal(lock.packages["node_modules/@qualitzer/mobile-gateway"].version, "1.0.6");
  ...
  assert.ok(/^http:\/\/192\.168\.1\.101:8791\/$/.test(url), "EXPECTED_LOCAL_DELIVERY_URL");
  const response = await fetch(`${url}qualitzer-tecnicos-1.0.17-android.apk`, { signal: AbortSignal.timeout(20000) });
  ...
  fs.writeFileSync(path.join(root, "artifacts/parent-assignment-delivery-1.0.17.json"), JSON.stringify(report, null, 2));
