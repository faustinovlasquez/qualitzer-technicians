...
- La única dirección configurable es `BACKEND_URL`, terminada en `/api`, en el `.env` local ignorado por Git. El proceso/CI puede suministrar esa misma variable; tiene prioridad sobre el archivo. Expo, Gradle y los verificadores derivan `/mobile` en el mismo servidor, conservando un posible prefijo de ruta. No configurar `EXPO_PUBLIC_GATEWAY_URL` por separado.
- Los procesos de Expo/Gradle reciben un entorno limitado a variables de sistema/toolchain, `NODE_ENV=production`, `EXPO_NO_DOTENV=1`, `EAS_BUILD_PROFILE=standalone-apk`, `EXPO_PUBLIC_STANDALONE=true`, `BACKEND_URL` validada y `EXPO_PUBLIC_GATEWAY_URL` derivada internamente. No heredan las demás variables de backend, branding local ni secretos del terminal. La clave Google se lee separadamente desde el `.env` de la app.
- Los perfiles de EAS no contienen dominios. Para un build remoto, suministrar `BACKEND_URL` mediante el entorno de EAS/CI correspondiente; `.env` no se versiona ni se incluye automáticamente por estar ignorado. Cambiar la dirección requiere generar una nueva APK, no editar el código ni las pruebas.
...
- El runner rechaza `expo-dev-client`, valida configuración embebida, firma APK v2 y certificado no-debug, identidad de paquete, ABIs, librerías Hermes, cabecera de bytecode, URL en `assets/app.config` coincidente con el entorno resuelto, flag standalone y actualizaciones remotas deshabilitadas. La app consume esa configuración embebida; no necesita otra copia del dominio en el bundle JavaScript. Inspecciona el manifest y calcula SHA-256.
