...
test("jornada initially selects pending and in-progress together and preserves manual filter choices", () => {
  ...
  const badge = (id: string) => elements<{ testID?: string; children: string }>(render(), "Text").find(node => node.props.testID === id)?.props.children;
  assert.equal(badge("assignment-type-count-works"), "3");
  assert.equal(badge("assignment-status-count-all"), "5");
  assert.equal(badge("assignment-status-count-pending"), "1");
  assert.equal(badge("assignment-status-count-in_progress"), "2");
  assert.equal(badge("assignment-status-count-completed"), "2");
  ...
  assert.equal(badge("assignment-type-count-works"), "2");
  assert.equal(badge("assignment-status-count-all"), "5");
...
for (const type of ["internal_maintenance", "external_ot"] as const) {
  ...
  test(`${type}: empty assigned parent remains visible and opens without a fabricated work`, () => {
    ...
    let tree = render();
    const badge = (id: string) => elements<{ testID?: string; children: string }>(render(), "Text").find(node => node.props.testID === id)?.props.children;
    const typeCountId = `assignment-type-count-${type === "internal_maintenance" ? "maintenances" : "orders"}`;
    assert.equal(badge(typeCountId), "1");
    assert.equal(badge("assignment-type-count-works"), "0");
    ...
    tab.props.onPress();
    tree = render();
    assert.equal(badge("assignment-status-count-all"), "1");
    assert.equal(badge("assignment-status-count-pending"), "1");
    assert.equal(badge("assignment-status-count-in_progress"), "0");
    assert.equal(badge("assignment-status-count-completed"), "0");
    ...
    assert.equal(badge(typeCountId), "0");
    assert.equal(badge("assignment-status-count-all"), "0");
    search.props.onChangeText("");
    props.offline = uiSnapshot();
    assert.equal(badge(typeCountId), "1+");
    assert.equal(badge("assignment-status-count-all"), "1+");
    assert.equal(badge("assignment-status-count-completed"), "—");
    props.offline = null;
    assert.equal(badge(typeCountId), "—");
    assert.equal(badge("assignment-status-count-all"), "—");
