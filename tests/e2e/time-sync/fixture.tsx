...
import type { AssignmentWork, Session, StatusInput } from "../../../src/domain/models";
import type { OfflineCommand, OfflineController, OfflineOperation } from "../../../src/domain/offline";
...
type Screen = "widget" | "invalid" | "creation" | "completion" | "blocked" | "notification" | "maintenance" | "sync";
...
function Fixture({ screen, client }: { screen: Screen; client: MobileNotificationClient | null }) {
  ...
  if (screen === "completion" || screen === "blocked") return <CompletionDialog maintenance={false} work={work} allowEditExecutionTime generatedAt={`${date}T12:00:00Z`} initialDate={date} range={range} reasons={[]} canSubmit={screen !== "blocked"} busy={false} error={null} mode="live" onClose={() => {}} onSubmit={input => { submitted.push(input); }} />;
