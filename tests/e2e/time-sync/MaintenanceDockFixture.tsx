...
declare global { interface Window { maintenanceDock: { inputs: CreationInput[]; failRead: boolean; failStatus: boolean; starts: number }; } }
export function MaintenanceDockFixture() {
  ...
  window.maintenanceDock ??= { inputs: [], failRead: false, failStatus: false, starts: 0 };
  ...
  return <OrderDetailScreen group={group} tenant={tenant} branchName="Taller" mode="live" busy={false} storageKey="maintenance-dock" range={{ startDate: "2026-09-23", endDate: "2026-09-23" }} companyBranchId={1} technicianName="Tecnico"
    ...
    onLoadDelivery={async () => {
      if (window.maintenanceDock.failStatus) throw new Error("No se pudo consultar el estado de la OT.");
      return { groupId: group.id, status: group.status, maintenanceType: "correctivo", finalizationNote: null, damageType: null, durationMinutes: null, startedAt: null, finalizedAt: null, incompleteChecklists: [], canStart: group.status === "pending", canTechnicianDeliver: true, technicianDeliverySupported: true };
    }}
