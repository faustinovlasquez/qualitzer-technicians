...
test("actual FileWorkspace labels demo listings separately, retains review copies and leaves live counts unchanged", async () => {
  ...
  const listed: Attachment[] = [remote, { ...remote, id: 42 }, ...operations.map(pendingDocumentAttachment)];
  ...
  assert.ok(demo.includes('[2," en listado demo"," · ",2," en cola"]'));
  ...
  assert.ok(live.includes('[2," confirmados"," · ",2," en cola"]'));
